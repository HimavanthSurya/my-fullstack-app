...
namespace eCommerceApp.Application.Services.Implementations
{
    public class ProductService(IGeneric<Product> productInterface, IMapper mapper) : IProductService
    {
        public async Task<ServiceResponse> AddAsync(CreateProduct product)
        {
            ...
            return (result > 0) ? new ServiceResponse(true, "Product Added Successfully") : new ServiceResponse(false, "Product Not Found");
        }

        public async Task<ServiceResponse> DeleteAsync(Guid id)
        {
            int result = await productInterface.DeleteAsync(id);
            return (result > 0) ? new ServiceResponse(true, "Product Deleted Successfully") : new ServiceResponse(false, "Product Not Found of failed to delete");
        }
