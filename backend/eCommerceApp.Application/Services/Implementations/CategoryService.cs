...
namespace eCommerceApp.Application.Services.Implementations
{
    public class CategoryService(IGeneric<Category> categoryInterface, IMapper mapper) : ICategoryService
    {
        public async Task<ServiceResponse> AddAsync(CreateCategory category)
        {
            ...
            return (result > 0) ? new ServiceResponse(true, "Category Added Successfully") : new ServiceResponse(false, "Category Not Found");
        }

        public async Task<ServiceResponse> DeleteAsync(Guid id)
        {
            int result = await categoryInterface.DeleteAsync(id);
            return (result > 0) ? new ServiceResponse(true, "Category Deleted Successfully") : new ServiceResponse(false, "Category Not Found of failed to delete");
        }
